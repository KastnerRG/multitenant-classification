...
def multi_proc_fft(app, seg_len, ffts_per_trace, detrend, half_trace, data_dir_train, data_dir_test, data_dir):
  print(app)
  count = 0 
  num_traces = 100
...
def generate_FFT(data_sets, seg_len, ffts_per_trace, detrend, half_trace, train_dir, test_dir):
    starttime = time.time()
    processes = []
    for run in data_sets: 
      p = multiprocessing.Process(target=multi_proc_fft, args=(run, seg_len, ffts_per_trace, detrend, half_trace, train_dir, test_dir, data_sets[run]))
      processes.append(p)
      p.start()
...
if __name__ == "__main__":
  ...
  for board in boards:
      ...
      data_dir_train = f"./segments/fft/{board}/Train_Segments_Pos_Detrend_8K/"
      data_dir_test = f"./segments/fft/{board}/Test_Segments_Pos_Detrend_8K/"
      ...
      generate_FFT(data_set, seg_len, ffts_per_trace, detrend, half_trace, data_dir_train, data_dir_test)
